...
package provide ::ms::HSP_HWB 0.1

# Create the mustang **HSP_HWB** package.
namespace eval ::ms::HSP_HWB {}

# Create an alias for the mustang **HSP_HWB** conversion command.
interp alias {} ::HSP_HWB {} ::ms::HSP_HWB::Conversion

## Conversion
#
# Transform **HSP** colors into **HWB** colors.
#
# Where:
#
...
#
#   and so on and so forth...
proc ::ms::HSP_HWB::Conversion { channels } {
    foreach { hue saturation perceived_brightness } $channels {
        # Check if it's a gray.
        switch -nocase -- $hue {
            UNDEF {
                set blackness [expr { 100.0-$perceived_brightness }]

                # It's a gray.
                lappend results UNDEF $perceived_brightness $blackness
            }
            default {
