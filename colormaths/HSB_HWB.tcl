...
package provide ::ms::HSB_HWB 0.1

# Create the mustang **HSB_HWB** package.
namespace eval ::ms::HSB_HWB {}

# Create an alias for the mustang **HSB_HWB** conversion command.
interp alias {} ::HSB_HWB {} ::ms::HSB_HWB::Conversion

## Conversion
#
# Transform **HSB** colors into **HWB** colors.
#
# Where:
#
...
#
#   and so on and so forth...
proc ::ms::HSB_HWB::Conversion { channels } {
    foreach { hue saturation brightness } $channels {
        # Compute the blackness [0,100.0].
        set blackness [expr { 100.0-$brightness }]

        # Check if it's a gray.
        switch -nocase -- $hue {
            UNDEF {
                # It's a gray.
                lappend results UNDEF $brightness $blackness
            }
            default {
                set s [expr { $saturation*0.01 }]; # range [0,1.0]
                set b [expr { $brightness*0.01 }]; # range [0,1.0]

                lappend results $hue [expr { (1.0-$s)*$b*100.0 }] $blackness
            }
        }
    }
