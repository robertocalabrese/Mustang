...
package provide ::ms::HSP_HSB 0.1

# Create the mustang **HSP_HSB** package.
namespace eval ::ms::HSP_HSB {}

# Create an alias for the mustang **HSP_HSB** conversion command.
interp alias {} ::HSP_HSB {} ::ms::HSP_HSB::Conversion

## Conversion
#
# Transform **HSP** colors into **HSB** colors.
#
# Where:
#
...
#
#   and so on and so forth...
proc ::ms::HSP_HSB::Conversion { channels } {
    foreach { hue saturation perceived_brightness } $channels {
        # Check if it's a gray.
        switch -nocase -- $hue {
            UNDEF {
                # It's a gray.
                lappend results UNDEF 0 $perceived_brightness
            }
            default {
                ...
                # Compute the brightness [0,100.0] and saturation [0,100.0].
