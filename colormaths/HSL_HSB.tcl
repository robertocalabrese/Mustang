...
package provide ::ms::HSL_HSB 0.1

# Create the mustang **HSL_HSB** package.
namespace eval ::ms::HSL_HSB {}

# Create an alias for the mustang **HSL_HSB** conversion command.
interp alias {} ::HSL_HSB {} ::ms::HSL_HSB::Conversion

## Conversion
#
# Transform **HSL** colors into **HSB** colors.
#
# Where:
#
...
#
#   and so on and so forth...
proc ::ms::HSL_HSB::Conversion { channels } {
    foreach { hue saturation lightness } $channels {
        # Check if it's a gray.
        switch -nocase -- $hue {
            UNDEF {
                # It's a gray.
                lappend results UNDEF 0 $lightness
            }
            default {
